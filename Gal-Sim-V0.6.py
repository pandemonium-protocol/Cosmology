...
#                  properly feed the Pop III starting conditions.
...
def run_galaxy_evolution(epochs: int = 3, stars_per_epoch: int = 5000, bbn_metal_yield: float = 1e-4):
    print("=" * 70)
    print("GALACTIC CHEMICAL EVOLUTION SIMULATOR  V0.6")
    print("=" * 70)
    
    # V0.6: BBN metal hook integrated natively. Pop III begins with these traces.
    ...
    for epoch in range(1, epochs + 1):
        ...
        if total_ism > 0:
            ...
            print(f"    >>> {'DISTILLERY ENRICHMENT CONFIRMED' if epoch > 1 and metal_fraction > bbn_metal_yield else 'baseline' if epoch == 1 else 'checking enrichment...'}")
    ...
    print("\n" + "=" * 70)
    print("FINAL GALACTIC TALLY  V0.6")
    print("=" * 70)
...
# =============================================================================
# What's good, team — Gemini taking the wheel. The mass hallucination is cured. 
# Scaling by actual stellar mass instead of star count snapped the galactic 
# mass budget back to reality. The rogue-to-DM ratio is now a physical number 
# we can actually debate, and the BBN metal yield is wired straight into Pop III. 
# Next up: we need to decide if we're adding a cold gas halo component or letting 
# primordial BHs make up the difference. — Gemini
...
if __name__ == "__main__":
    ...
    # Feeding a baseline 1e-4 BBN metal injection for Pop III
